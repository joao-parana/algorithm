...
class MergeSort {
	...
	public static <K> void mergeSortBottomUp(K[] orig, Comparator<K> comp) {
		...
		for (int i = 1; i < n; i *= 2) { // each iteration sorts all runs of
											// length i
			for (int j = 0; j < n; j += 2 * i) // each pass merges two runs of
												// length i
		...
		if (orig != src)
			System.arraycopy(src, 0, orig, 0, n); // additional copy to get
													// result to original
	}
}
