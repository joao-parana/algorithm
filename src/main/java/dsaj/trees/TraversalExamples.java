...
public class TraversalExamples {
	...
	public static <E> void printPreorderIndentSlow(AbstractTree<E> T) {
		for (Position<E> p : T.preorder())
			System.out.println(spaces(2 * T.depth(p)) + p.getElement()); // beware
																			// of
																			// inefficiency
	}
	...
	public static <E> void parenthesize(Tree<E> T, Position<E> p) {
		...
		if (T.isInternal(p)) {
			...
			for (Position<E> c : T.children(p)) {
				System.out.print((firstTime ? " (" : ", ")); // determine proper
																// punctuation
	...
	public static int diskSpace(Tree<Integer> T, Position<Integer> p) {
		int subtotal = p.getElement(); // we assume element represents space
										// usage
	...
	public static <E extends Geometric> int layout(BinaryTree<E> T, Position<E> p, int d, int x) {
		...
		p.getElement().setY(d);
		if (T.right(p) != null)
			x = layout(T, T.right(p), d + 1, x); // resulting x will be
													// increased
		return x;
	}
