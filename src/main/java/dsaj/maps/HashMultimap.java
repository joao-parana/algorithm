...
public class HashMultimap<K, V> {
	...
	void put(K key, V value) {
		...
		if (secondary == null) {
			...
			map.put(key, secondary); // begin using new list as secondary
										// structure
		}
		secondary.add(value);
		total++;
		...
		if (secondary != null) {
			...
			if (wasRemoved) {
				...
				if (secondary.isEmpty())
					map.remove(key); // remove secondary structure from primary
										// map
			}
		}
		return wasRemoved;
	...
	Iterable<V> removeAll(K key) {
		...
		if (secondary != null) {
			...
			secondary = new ArrayList<>(); // return empty list of removed
											// values
		return secondary;
	}
